extends CharacterBody2D

class_name EnemyLinear

# ── Signals ───────────────────────────────────────────────────────────────────
signal enemy_destroyed

# ── Constants ─────────────────────────────────────────────────────────────────
const ENEMY_COLOR: Color = Color(1.0, 0.267, 0.267)  # #ff4444 red
const DIAMOND_SIZE: float = 14.0
const BASE_SPEED: float = 90.0
const SHOOT_INTERVAL: float = 2.5

# ── State ─────────────────────────────────────────────────────────────────────
var _base_velocity: Vector2 = Vector2(0.0, BASE_SPEED)
var _gravity_deviation: Vector2 = Vector2.ZERO
var _shoot_timer: float = 0.0
var _is_active: bool = true
var _game_manager: Node = null
var _enemy_bullets_container: Node2D = null
var _enemy_bullet_script: Script = null
var _enemy_bullet_speed: float = 180.0

# ── Lifecycle ─────────────────────────────────────────────────────────────────
func _ready() -> void:
	_game_manager = get_node_or_null("/root/Main")
	if is_instance_valid(_game_manager):
		_enemy_bullets_container = _game_manager.get_enemy_bullets_container()
		_enemy_bullet_speed = _game_manager.get_setting("enemy_bullet_speed", 180.0) as float

	_enemy_bullet_script = load("res://scripts/entities/enemy_bullet.gd") as Script

	# Collision shape — diamond approximated with capsule
	var col: CollisionShape2D = CollisionShape2D.new()
	var shape: CapsuleShape2D = CapsuleShape2D.new()
	shape.radius = DIAMOND_SIZE * 0.5
	shape.height = DIAMOND_SIZE
	col.shape = shape
	add_child(col)

	# Enemy on layer 4, detects player bullets (layer 3) — collision with player handled via Area2D hitbox
	collision_layer = 8
	collision_mask = 4

	# Hitbox Area2D for detecting player collision
	var hitbox: Area2D = Area2D.new()
	hitbox.name = "Hitbox"
	hitbox.collision_layer = 16
	hitbox.collision_mask = 2   # Player layer
	var hb_shape: CollisionShape2D = CollisionShape2D.new()
	var hb_circle: CircleShape2D = CircleShape2D.new()
	hb_circle.radius = DIAMOND_SIZE * 0.6
	hb_shape.shape = hb_circle
	hitbox.add_child(hb_shape)
	add_child(hitbox)
	hitbox.body_entered.connect(_on_hitbox_body_entered)

	# Random shoot offset to avoid synchronized fire
	var rng: RandomNumberGenerator = RandomNumberGenerator.new()
	rng.randomize()
	_shoot_timer = rng.randf_range(0.5, SHOOT_INTERVAL)

func _physics_process(delta: float) -> void:
	if not _is_active:
		return

	# Apply base velocity plus gravitational deviation
	velocity = _base_velocity + _gravity_deviation * BASE_SPEED
	# Decay deviation each frame
	_gravity_deviation = _gravity_deviation.lerp(Vector2.ZERO, 3.0 * delta)
	move_and_slide()
	queue_redraw()

	# Shooting
	_shoot_timer -= delta
	if _shoot_timer <= 0.0:
		_shoot_timer = SHOOT_INTERVAL
		_fire()

	# Self-destroy when off-screen (below viewport)
	var vp: Viewport = get_viewport()
	if is_instance_valid(vp):
		if position.y > vp.get_visible_rect().size.y + 64.0:
			_is_active = false
			enemy_destroyed.emit()
			queue_free()

func _draw() -> void:
	# Draw diamond shape
	var top: Vector2 = Vector2(0.0, -DIAMOND_SIZE)
	var right: Vector2 = Vector2(DIAMOND_SIZE, 0.0)
	var bottom: Vector2 = Vector2(0.0, DIAMOND_SIZE)
	var left: Vector2 = Vector2(-DIAMOND_SIZE, 0.0)
	var points: PackedVector2Array = PackedVector2Array([top, right, bottom, left])
	draw_colored_polygon(points, ENEMY_COLOR)

func apply_gravity_deviation(deviation: Vector2) -> void:
	_gravity_deviation += deviation

func take_hit() -> void:
	if not _is_active:
		return
	_is_active = false
	enemy_destroyed.emit()
	queue_free()

func _fire() -> void:
	if not is_instance_valid(_enemy_bullets_container):
		return
	if not is_instance_valid(_enemy_bullet_script):
		return
	var bullet: Area2D = Area2D.new()
	bullet.set_script(_enemy_bullet_script)
	bullet.position = global_position + Vector2(0.0, DIAMOND_SIZE)
	_enemy_bullets_container.add_child(bullet)
	(bullet as EnemyBullet).setup(Vector2(0.0, 1.0), _enemy_bullet_speed)

func _on_hitbox_body_entered(body: Node2D) -> void:
	# body is the player (CharacterBody2D with player.gd)
	if body is Player:
		(body as Player).die()
