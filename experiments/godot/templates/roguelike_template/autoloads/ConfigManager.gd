...
# Must be declared FIRST in the autoload list — all other singletons depend on it.
# =============================================================================
...
# --- Typed getters -----------------------------------------------------------
# Fallback values are intentionally neutral so a missing config causes
# visible inactivity rather than silently wrong values.
...
## Reads a hex color from the nested config["colors"] dictionary.
...
## Returns the full raw config Dictionary.
## Use only when a subsystem (e.g. DungeonGenerator) needs batch access
## to many keys and calling individual typed getters would be verbose.
